...
public class JSONPreSignBatchParser {
	...
	public static PresignBatch parseFromJSON(final byte[] preSignJson) throws JSONException {
		...
		try {
			jsonObject = new JSONObject(new JSONTokener(new String(preSignJson)));
	...
	private static List<BatchDataResult> parseBatchDataResults(final JSONArray jsonResults) throws JSONException {
		...
		for (int i = 0; i < jsonResults.length(); i++) {
			...
			try {
				result = BatchDataResult.Result.valueOf(dataResult.getString(JSELEM_RESULT));
			}
